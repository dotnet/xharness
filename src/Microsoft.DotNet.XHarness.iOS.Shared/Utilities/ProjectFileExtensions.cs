...
namespace Microsoft.DotNet.XHarness.iOS.Shared.Utilities
{
    public static class ProjectFileExtensions
    {
        ...
        public static async Task<string> GetPropertyByMSBuildEvaluationAsync(this XmlDocument csproj, IProcessManager processManager, string projectPath, string evaluateProperty, string dependsOnTargets = "", Dictionary<string, string> properties = null)
        {
            var xml =
$@"<Project DefaultTargets='WriteProperty' xmlns='http://schemas.microsoft.com/developer/msbuild/2003'>
	...
	<Target Name='WriteProperty' DependsOnTargets='{dependsOnTargets}'>
		<PropertyGroup>
			<_Properties>$({evaluateProperty})</_Properties>
		</PropertyGroup>
...
";
            ...
            try
            {
                File.WriteAllText(inspector, xml);
                ...
                var rv = await processManager.RunAsync(proc, log, environmentVariables: env, timeout: TimeSpan.FromSeconds(15));
                if (!rv.Succeeded)
                {
                    throw new Exception($"Unable to evaluate the property {evaluateProperty}. Log:{Environment.NewLine}{log}");
                }
