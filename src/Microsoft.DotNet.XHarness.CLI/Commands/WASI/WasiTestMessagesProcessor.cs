...
public class WasiTestMessagesProcessor
{
    ...
    private readonly WasiSymbolicatorBase? _symbolicator;
    ...
    // Set once `WASI EXIT` message is received
    public TaskCompletionSource WasiExitReceivedTcs { get; } = new ();

    public WasiTestMessagesProcessor(string xmlResultsFilePath, string stdoutFilePath, ILogger logger, string? errorPatternsFile, WasiSymbolicatorBase? symbolicator)
    {
    ...
    {
        WasiLogMessage? logMessage = null;
        string line;
        ...
        if (message.StartsWith("{"))
        {
            try
            {
                logMessage = JsonSerializer.Deserialize<WasiLogMessage>(message);
        ...
        if (line.StartsWith("WASI EXIT"))
        {
            _logger.LogDebug("Reached wasi exit");
            if (!WasiExitReceivedTcs.TrySetResult())
