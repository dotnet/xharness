...
internal class WasiBrowserTestRunner
{
    private readonly WasiTestBrowserCommandArguments _arguments;
    private readonly ILogger _logger;
    private readonly IEnumerable<string> _passThroughArguments;
    private readonly WasiTestMessagesProcessor _messagesProcessor;
    ...
    public async Task<ExitCode> RunTestsWithWebDriver(DriverService driverService, IWebDriver driver)
    {
        ...
        try
        {
            ...
            TaskCompletionSource wasiExitReceivedTcs = _messagesProcessor.WasiExitReceivedTcs;
            var tasks = new Task[]
            {
                    wasiExitReceivedTcs.Task,
                    consolePumpTcs.Task,
            ...
            if (task == wasiExitReceivedTcs.Task && wasiExitReceivedTcs.Task.IsCompletedSuccessfully)
            {
