...
{
    /// <summary>
    /// Common ancestor for `run` and `just-run` orchestrators.
    /// </summary>
    public abstract class BaseRunOrchestrator : BaseOrchestrator
    {
        ...
        protected async Task<ExitCode> ExecuteApp(
        ...
        protected async Task<ExitCode> ExecuteMacCatalystApp(
            AppBundleInformation appBundleInfo,
            TimeSpan timeout,
            int expectedExitCode,
            IReadOnlyCollection<(string, string)> environmentalVariables,
