...
public abstract class WasiSymbolicatorBase
{
    ...
    public static WasiSymbolicatorBase? Create(Type? symbolicatorType, string? symbolMapFile, string? symbolsPatternFile, ILogger logger)
    {
        ...
        var symbolicator = Activator.CreateInstance(symbolicatorType) as WasiSymbolicatorBase;
        if (symbolicator is null)
        {
            // should not happen
            logger.LogError($"Symbolicator '{symbolicatorType}' is not of WasiSymbolicatorBase type.");
            return null;
        }
