...
namespace Microsoft.DotNet.XHarness.Apple.Tests.AppOperations
{
    public class InstallOrchestratorTests
    {
        ...
        private Mock<ISimulatorDevice>? _simulator;
        private Mock<ISimulatorDevice>? _device;
        ...
        public async Task OrchestrateSimulatorInstallationTest()
        {
            ...
            _appUninstaller
                .Setup(x => x.UninstallApp(_simulator!.Object, s_appIdentifier, It.IsAny<CancellationToken>()))
                .ReturnsAsync(new ProcessExecutionResult
                {
        ...
        private void SetupMockSimulator(string osVersion = "14.4")
        {
            _simulator = new();
            _simulator.Setup(x => x.UDID).Returns(UDID);
            _simulator.Setup(x => x.Name).Returns(DeviceName);
            _simulator.Setup(x => x.OSVersion).Returns(osVersion);
        ...
        [MemberNotNullWhen(true, "_device")]
        private void SetupMockDevice(string osVersion = "14.4")
        {
            _device = new();
            _device.Setup(x => x.UDID).Returns(UDID);
            _device.Setup(x => x.Name).Returns(DeviceName);
            _device.Setup(x => x.OSVersion).Returns(osVersion);
