...
Keep the `COPILOT_GITHUB_TOKEN` expression on one line so current `gh-aw` versions generate valid workflow YAML. `gh-aw compile` automatically wires `pat_pool` into the activation, agent, and detection jobs' `needs:` graphs because of the `needs.pat_pool.` references within the `engine.env` property.
